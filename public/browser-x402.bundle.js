    ...
    Message = class _Message {
      ...
      static from(buffer) {
        ...
        for (let i = 0; i < accountCount; i++) {
          const account2 = guardedSplice(byteArray, 0, PUBLIC_KEY_LENGTH);
          accountKeys.push(new PublicKey(import_buffer2.Buffer.from(account2)));
        }
    ...
    MessageV0 = class _MessageV0 {
      ...
      resolveAddressTableLookups(addressLookupTableAccounts) {
        ...
        for (const tableLookup of this.addressTableLookups) {
          const tableAccount = addressLookupTableAccounts.find((account2) => account2.key.equals(tableLookup.accountKey));
          if (!tableAccount) {
...
// node_modules/@acedatacloud/x402-client/dist/evm.js
function randomNonce32() {
  const bytes = new Uint8Array(32);
  globalThis.crypto.getRandomValues(bytes);
  return "0x" + Array.from(bytes, (b) => b.toString(16).padStart(2, "0")).join("");
}
function buildTypedData(requirements, authorization) {
  return {
    types: {
...
async function signEVMPayment(requirements, provider, address) {
  ...
  const payload = { authorization, signature: signature2 };
  return {
    x402Version: 2,
...
// node_modules/@acedatacloud/x402-client/dist/solana.js
var TOKEN_PROGRAM_ID = "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA";
var ATA_PROGRAM_ID = "ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL";
async function loadWeb3() {
  return Promise.resolve().then(() => (init_index_browser_esm(), index_browser_esm_exports));
}
function getPublicKeyString(wallet) {
  return typeof wallet.publicKey.toBase58 === "function" ? wallet.publicKey.toBase58() : wallet.publicKey.toString();
}
async function findATA(owner, mint) {
  const { PublicKey: PublicKey2 } = await loadWeb3();
  const [ata] = PublicKey2.findProgramAddressSync([new PublicKey2(owner).toBytes(), new PublicKey2(TOKEN_PROGRAM_ID).toBytes(), new PublicKey2(mint).toBytes()], new PublicKey2(ATA_PROGRAM_ID));
  return ata.toBase58();
}
...
async function signSolanaPayment(requirements, wallet) {
  const { PublicKey: PublicKey2, Transaction: Transaction2, TransactionInstruction: TransactionInstruction2, ComputeBudgetProgram: ComputeBudgetProgram2, Connection: Connection2 } = await loadWeb3();
  ...
  if (computeUnitLimit > 0) {
    tx.add(ComputeBudgetProgram2.setComputeUnitLimit({ units: computeUnitLimit }));
  }
  if (computeUnitPrice > 0) {
    tx.add(ComputeBudgetProgram2.setComputeUnitPrice({ microLamports: computeUnitPrice }));
  }
  tx.add(new TransactionInstruction2({
    programId: new PublicKey2(TOKEN_PROGRAM_ID),
    keys: [
      { pubkey: new PublicKey2(sourceATA), isSigner: false, isWritable: true },
      { pubkey: new PublicKey2(mint), isSigner: false, isWritable: false },
      { pubkey: new PublicKey2(destATA), isSigner: false, isWritable: true },
      { pubkey: new PublicKey2(payerAddress), isSigner: true, isWritable: false }
    ],
    data: Buffer.from(buildTransferCheckedData(amount, decimals))
  }));
  tx.feePayer = new PublicKey2(payerAddress);
  ...
  const payload = { signature: signature2 };
  return {
    x402Version: 2,
    scheme: requirements.scheme || "exact",
...
// node_modules/@acedatacloud/x402-client/dist/sdkAdapter.js
function encodePaymentHeader(envelope) {
...
function createX402PaymentHandler(options) {
  ...
  return async function x402PaymentHandler(ctx) {
    const requirement = selectRequirement(ctx.accepts, network);
    if (!requirement) {
      const available = ctx.accepts.map((a) => a.network).join(", ") || "<none>";
      throw new Error(`X402: no payment requirement for network "${network}". Available: ${available}`);
    }
    let envelope;
    if (network === "solana") {
...
async function runPaidService(service, body, paymentHandler) {
  ...
  const usdc = Number(baseRequirement.maxAmountRequired) / 1e6;
      ...
      messages: [
        ...
        },
        {
          ...
          content: [
            ...
            `Search evidence: ${JSON.stringify(search).slice(0, 5e3)}`,
            `Extraction evidence: ${JSON.stringify(extraction).slice(0, 5e3)}`
          ].join("\n")
        }
      ],
      max_tokens: 600
